...
| Feature | What it does | Enabled by | Where | Why it helps |
| --- | --- | --- | --- | --- |
| **Vertical slices** | Each operation's command, handler, result and validator live together | Folder-per-operation layout | `Application/Features/Products/<Operation>/` | A change to one operation touches one folder |
