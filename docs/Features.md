...
| **Audit behaviour** | Records one audit event per request that opts in, whatever came back (including the outcomes authorization and validation short-circuit to), with the real actor and the target; fail closed or best effort per request | `IAuditableRequest<TResponse>` (action, principal, failure policy, `DescribeAudit`) | `AuditBehavior.cs`, `Common/Auditing/` | A security-relevant action is recorded once, uniformly, without per-handler code, and the union case name is the outcome |
...
| **Fixed pipeline order** | Logging, then audit, then authorization, then validation, then transaction | Registration order | `Application/DependencyInjection.cs` | Cheap checks first; nothing runs a transaction for a request that was going to be rejected, and the audit behavior outside them records the requests they refuse |
...
| **Impersonation** | `POST /api/v1/impersonation/tokens` lets an `Administrator` or `Support` caller mint a short-lived token acting as another identity, in every environment. A reason (10 to 500 characters) is mandatory and recorded; no chained impersonation; only assignable roles, and a non-administrator may grant only roles they hold; the lifetime is capped; a configuration switch turns it off (404) | A normal command slice with the `Impersonator` policy (`AdministratorRequirement("Administrator", "Support")`), `IImpersonationTokenIssuer` implemented in Api, a separate signing key accepted by the bearer scheme alongside the ordinary one, `ImpersonationOptions` (validated on start), RFC 8693 `act` plus `impersonated` and `imp_reason` claims | `Application/Features/Impersonation/IssueToken/`, `Application/Common/Authorization/ImpersonationClaims.cs`, `Api/Impersonation/`, `Api/Controllers/ImpersonationController.cs` | Support can reproduce what a user sees, accountably, without their credentials. It is a controlled authentication bypass, so the role gate, mandatory reason, separate key, no chaining, lifetime cap and off switch are hard requirements. Every attempt, including those refused before the handler, is recorded in the audit stream, and a token is never returned unrecorded |
| **Audit stream** | A separate, append-only record of security-relevant actions: every impersonation token attempt (issued or refused, by the handler, the policy or validation), every request made under an impersonation token (with the token's `jti`, which ties it to the mint), and the four product mutations, each with the real actor, the target, the outcome, the reason, the trace id and the source address; never a token, secret, `Authorization` header or body. Impersonation is fail closed (no token is returned if its record cannot be written); the rest is best effort with an Error logged. Not audited: anonymous and failed-authentication requests. The application never deletes an audit file | `IAuditLog` (Application) with `FileAuditLog` in Api: JSON Lines, one file per UTC day (`audit-yyyyMMdd.jsonl`), lock-serialized and flushed per event, throwing on IO failure; `AuditBehavior` and `IAuditableRequest`; `ImpersonationAuditMiddleware`; `AuditOptions` (`Audit:Directory`, no off switch) | `Application/Common/Auditing/`, `Application/Common/Behaviors/AuditBehavior.cs`, `Api/Audit/` | Who did what, to what and why, kept apart from diagnostic logs (never sampled, filtered or mixed in) and honest about write failure. A database table can replace the file behind `IAuditLog` without changing a caller |
