...
/// declares. Shared case types are deliberately meaning-free (an arbitrary <see cref="Results.Error"/>,
/// <see cref="Results.Success"/>, or a dev's own shared record can be reused across unions to mean
/// whatever that union's author wants); classification lives on the union instead, one union at a
/// time. Because <see cref="ShouldCommit"/> must be implemented as a <c>switch</c> over the union's
/// own <c>this</c> value, the C# compiler's union-exhaustiveness check forces every one of that
/// union's case types to be classified — including a case type added after this method was first
/// written. There is no default branch to silently fall through: a union with an unhandled case
/// fails to compile.
/// </summary>
